...
public:
    ...
    search_result_t continue_search(search_val_pt search_val_p, rx_compare_type ct);
    void search_str(const std::string &str, const std::string &encoding = "UTF-8");
    pid_t target_pid();
    mach_port_t target_task();
    void set_include_readonly(boolean_t include_readonly);
private:
    ...
    void init_regions();
    void free_regions();
    inline boolean_t should_skip_region(const region_t &region);
private:
    pid_t                       _target_pid;
