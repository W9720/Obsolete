...
-(BOOL) dumpDecryptedImage:(const struct mach_header *)image_mh fileName:(const char *)encryptedImageFilenameStr image:(int)imageNum task:(vm_map_t)targetTask{
    ...
    struct mach_header header = {0};
    vm_region_basic_info_data_64_t region_info = {0};
    mach_vm_address_t imageAddress = (mach_vm_address_t)image_mh;
    if (readmem((mach_vm_offset_t*)&header, imageAddress, sizeof(struct mach_header), targetTask, &region_info))
    {
        ...
        @throw [NSException exceptionWithName:@"ReadMemoryFailed"
                                       reason:@"Can't read header from target process"
                                     userInfo:nil];
        return false;
    }
    ...
    // sleep(1);exit(1);
    // Use the header we read from memory
	...
	for (i=0; i<header.ncmds; i++) {
		if (lc->cmd == LC_ENCRYPTION_INFO || lc->cmd == LC_ENCRYPTION_INFO_64) {
            ...
            NSLog(@"[dumpDecrypted] aslr_slide= 0x%llx", aslr_slide);
...
-(void) dumpDecrypted:(pid_t)pid {
    ...
    if (kr != KERN_SUCCESS)
    {
        NSLog(@"[dumpDecrypted] Can't execute task_for_pid! Error: %d. Do you have the right permissions/entitlements?\n", kr);
        @throw [NSException exceptionWithName:@"TaskForPidFailed"
                                       reason:[NSString stringWithFormat:@"task_for_pid failed with error %d", kr]
                                     userInfo:nil];
        return;
    }
    ...
    if(kr != KERN_SUCCESS) {
        NSLog(@"[dumpDecrypted] task_info failed with error: %d", kr);
        @throw [NSException exceptionWithName:@"TaskInfoFailed"
                                       reason:[NSString stringWithFormat:@"task_info failed with error %d", kr]
                                     userInfo:nil];
        return;
    }
...
-(NSString *)IPAPath {
    // 创建解密文件夹路径
    NSString *decryptedDir = [NSString stringWithFormat:@"%@/解密文件", [self docPath]];

    // 确保解密文件夹存在
    NSFileManager *fm = [NSFileManager defaultManager];
    if (![fm fileExistsAtPath:decryptedDir]) {
        [fm createDirectoryAtPath:decryptedDir withIntermediateDirectories:YES attributes:nil error:nil];
    }

    // 使用应用名称和简短时间戳创建唯一的文件名
    NSDateFormatter *formatter = [[NSDateFormatter alloc] init];
    [formatter setDateFormat:@"MMdd_HHmm"];
    NSString *timestamp = [formatter stringFromDate:[NSDate date]];

    NSString *appName = self->appDirName ? [NSString stringWithUTF8String:self->appDirName] : @"Unknown";
    NSString *fileName = [NSString stringWithFormat:@"%@_%@.ipa", appName, timestamp];

    return [NSString stringWithFormat:@"%@/%@", decryptedDir, fileName];
}
