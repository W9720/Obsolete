...
- (NSString *)value
{
    ...
    switch (_valueType) {
        ...
        case VMMemValueTypeStr:{            // Str
            // 将十六进制字符串转换为字符数组
            NSMutableString *result = [NSMutableString string];
            NSString *hexStr = _value_16;
...
- (NSString *)permissionString {
    NSMutableString *permissions = [NSMutableString stringWithString:@"["];
    
    if (_protection & VM_PROT_READ) {
        [permissions appendString:@"R"];
    } else {
        [permissions appendString:@"-"];
    }
    
    if (_protection & VM_PROT_WRITE) {
        [permissions appendString:@"W"];
    } else {
        [permissions appendString:@"-"];
    }
    
    if (_protection & VM_PROT_EXECUTE) {
        [permissions appendString:@"X"];
    } else {
        [permissions appendString:@"-"];
    }
