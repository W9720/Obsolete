...
BOOL IsHUDEnabled(void)
{
    ...
    //NSLog(@"[yiming] spawned %s -check pid = %d", executablePath, task_pid);
    ...
    do {
        if (waitpid(task_pid, &status, 0) != -1)
        {
            ...
            //NSLog(@"[yiming] child status %d", WEXITSTATUS(status));
        }
...
#define LAUNCH_DAEMON_PATH ROOT_PATH("/Library/LaunchDaemons/com.yiming.hudapp.plist")
...
//        log_debug(OS_LOG_DEFAULT, "spawned %{public}s -exit pid = %{public}d", executablePath, task_pid);
        //NSLog(@"[yiming] spawned %s -exit pid = %d", executablePath, task_pid);
    ...
    if (isEnabled)
    {
        ...
        //NSLog(@"[yiming] spawned %s -hud pid = %d", executablePath, task_pid);
    }
    else
    {
        [NSThread sleepForTimeInterval:FADE_OUT_DURATION];
