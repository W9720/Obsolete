...
- (instancetype)init
{
    if (self = [super init])
    {
        ...
        do {
            ...
            if (!dispatcher)
            {
                //NSLog(@"[yiming] failed to get ivar _eventDispatcher");
                break;
            }
            //NSLog(@"[yiming] got ivar _eventDispatcher: %p", dispatcher);
            ...
            else
            {
                ...
                if (!runMethodIMP)
                {
                    //NSLog(@"[yiming] failed to get - [UIApplication _run] method");
                    break;
                }

                uint32_t *runMethodPtr = (uint32_t *)make_sym_readable((void *)runMethodIMP);
                //NSLog(@"[yiming] - [UIApplication _run]: %p", runMethodPtr);
